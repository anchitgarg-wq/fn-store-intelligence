...
if len(_whinv):
    ...
    # WH stock VALUE from the ERP's Inventory Value column (authoritative; not a Unit Cost recompute).
    # This is WH-tile only and does not affect FN's store stock-value logic, which is unchanged.
    _whinv['_wv'] = pd.to_numeric(_whinv['Inventory Value'], errors='coerce').fillna(0)
    wh_country_qty = _whinv.groupby('Country')['_wq'].sum().to_dict()
    wh_country_val = _whinv.groupby('Country')['_wv'].sum().to_dict()
    wh_cc_qty = _whinv.groupby(['Country','Key'])['_wq'].sum().to_dict()   # (country, colorcode) -> qty
    wh_cc_val = _whinv.groupby(['Country','Key'])['_wv'].sum().to_dict()   # (country, colorcode) -> value
    wh_total_qty = float(_whinv['_wq'].sum())
    wh_total_val = float(_whinv['_wv'].sum())
    print('WH stock (merchandise): total qty=%d | value=AED %s | countries=%s'
          % (int(wh_total_qty), f'{wh_total_val:,.0f}', sorted(wh_country_qty)))
else:
    wh_country_qty = {}; wh_country_val = {}; wh_cc_qty = {}; wh_cc_val = {}
    wh_total_qty = 0.0; wh_total_val = 0.0
...
def _whcell(qty, val):
    """WH tile payload: {qty, value}. Value is the headline on the tile, units beneath."""
    return {'qty': _r0(qty), 'value': _r0(val)}
wh_kpi = {'All Countries': _whcell(wh_total_qty, wh_total_val)}
for _cty,_q in wh_country_qty.items():
    wh_kpi[_cty] = _whcell(_q, wh_country_val.get(_cty, 0.0))
if wh_cc_qty:
    for _loc,_sub in g.groupby('Location'):
        ...
        _tq = 0.0; _tv = 0.0
        for _k in _held:
            _tq += wh_cc_qty.get((_cty,_k), 0.0)
            _tv += wh_cc_val.get((_cty,_k), 0.0)
        wh_kpi[_loc] = _whcell(_tq, _tv)
